...
class Spreadsheet
	...
	def getCliente(temp_SKU)#String
	...
	def getCantidad(temp_SKU)#String
		if @SKU.include? temp_SKU.to_i
	...
	def getUtilizado(temp_SKU)#String
		if @SKU.include? temp_SKU.to_i
	...
	def totalReservado(sku)
		temp=0
		for i in 0..@productos-1
			if @SKU[i].to_s==sku
				temp=temp+@Cantidad[i].to_i-@Utilizado[i].to_i
			end
		end
		return temp
	end

	def getDisponible(sku, id_Cliente)
		for i in 0..@productos
			if @SKU[i].to_s==sku and @Cliente[i].to_s == id_Cliente
				return @Cantidad[i].to_i - @Utilizado[i].to_i
			end
		end  
		return 0
	end
