...
=end
	...
	def despacharProductosOtraBodega(sku, cantidad, idalmacen)
		puts "Despachando productos  a otra bodega..."
				 	...
				 	if stocksalmacen[j]["_id"]== sku
				 		...
				 		for k in 0 .. productoadespachar.size()-1
					 		if(getSpaceBodega("despacho")>0  )
						 		...
						 		moverStockBodega(sku, cantidad)
						 		contador= contador+1
						 		puts contador. to_i
						 		puts cantidad.to_i
						 		if(contador.to_i == cantidad.to_i)
						 			puts ("despacho completo SKU: #{sku}")	
						 			return cantidad.to_i - contador.to_i
						 		end
							else 
...
#puts gestor.despacharProductosOtraBodega(1234567, 1, 123).to_s
